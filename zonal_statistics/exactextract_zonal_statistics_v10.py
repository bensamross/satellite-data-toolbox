...
def calculate():
    ...
    failure_csv_path = r"data/zonal_stats_v10_failures.csv"
    retry_rows = []
    failure_count = 0
    ...
    for time_range in tqdm.tqdm(time_ranges, desc='Human defined time ranges'):
        try:
            ...
            df_output.to_csv(f"data/zonal_stats_v10_{time_range.replace('/', '_')}_{sensor_name}.csv", index=False)
    ...
    if retry_rows:
        ...
        retry_df.to_csv("data/zonal_stats_v10_retry_queue.csv", index=False)
        print(f"Retry queue written to zonal_stats_v10_retry_queue.csv with {len(retry_df)} items")
